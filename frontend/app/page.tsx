              ...
              {messages.length === 0 ? (
                ...
                <div className="mx-auto max-w-3xl px-5 py-8">
                  {messages.map(message => <article key={message.id} className="mb-8 flex gap-4">
                    ...
                    <div className="min-w-0 pt-1"><p className="mb-1 text-xs font-semibold text-zinc-500">{message.role === "agent" ? "MyMinion" : "You"}</p><p className="whitespace-pre-wrap text-[15px] leading-7 text-zinc-700">{message.text}</p>{message.result && hasResults(message.result) && <div className="mt-4"><div className="mb-2 flex flex-wrap gap-2"><TraceBadge text={`Moss recalled ${message.result.memories_used.length}`}/><TraceBadge text={`Bright Data sources ${message.result.research.length}`}/><TraceBadge text={`Moss saved ${message.result.memories_saved.length}`}/></div><button onClick={() => setActiveResult(message.result ?? null)} className="rounded-lg border border-[#e4d36e] bg-[#fff9d9] px-3 py-2 text-xs font-bold text-[#675400] hover:bg-[#fff2a8]">Open mission receipt</button></div>}</div>
                  </article>)}
...
function ResultsPanel({ result, close }: { result: AgentResponse; close: () => void }) {
  ...
  return <aside className="fixed inset-y-0 right-0 z-40 w-[min(96vw,560px)] shrink-0 overflow-y-auto border-l border-zinc-200 bg-[#f7f3ea] p-5 text-zinc-900 shadow-2xl xl:static xl:z-auto xl:w-[520px] xl:shadow-none">
    <div className="flex items-center justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#806800]">{result.use_case.replaceAll("_", " ")}</p><h2 className="mt-1 font-black">Mission results</h2></div><Button variant="ghost" size="icon" onClick={close}><PanelRightClose size={17}/></Button></div>
...
function TraceBadge({ text }: { text: string }) { return <span className="rounded-full border border-zinc-200 bg-white px-2.5 py-1 text-[9px] font-bold text-zinc-500">{text}</span>; }
